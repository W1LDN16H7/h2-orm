...
public class EntityManagerProvider {
    ...
    /**
     * Close current thread's EntityManager - deprecated in favor of per-transaction EntityManagers
     */
    public static void closeEntityManager() {
        // This method is now deprecated since we create fresh EntityManagers per transaction
        // Left for backward compatibility but does nothing
        logger.debug("closeEntityManager() called - no operation needed with current transaction model");
    }
    ...
    public static <T> T executeInTransaction(String configName, TransactionCallback<T> callback) {
        ...
        try {
            // Get a fresh EntityManager for each transaction
            EntityManagerFactory factory = factories.get(configName);
            if (factory == null) {
                throw new DatabaseNotInitializedException();
            }
